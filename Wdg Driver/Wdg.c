...
for (uint8 i = 0; i < WDG_CONFIGURED_CHANNLES; i++)
   ...
   *(volatile uint32 *)((volatile uint8 *)Wdg_Ptr + WDTLOAD_OFFSET_ADDRESS) = Wdg_ConfigPtr[i].InitialTimeout;
...
void Wdg_SetTriggerCondition( uint16 timeout )
{
	...
	if (WDG_CONFIGURED_CHANNLES <= Wdg_Id)
	{
          ...
          for (uint8 i = 0; i < WDG_CONFIGURED_CHANNLES; i++)
            {
                switch(Wdg_ConfigPtr[i].Wdg_Id)
                      {
                          case 0:    Wdg_Ptr = &WATCHDOG0_BASE_ADDRESS;
                                     break;
                          case 1:    Wdg_Ptr = &WATCHDOG1_BASE_ADDRESS;
                                     break;
                      }
            }    
...
void WATCHDOG0_Handler(void)
{
  ...
  if(WdgEnNotification[0] != NULL_PTR)
  {
    /*Call the funciton*/
    WdgEnNotification[0]();
  }
...
void WATCHDOG1_Handler(void)
{
  ...
  if(WdgEnNotification[1] != NULL_PTR)
  {
    /*Call the funciton*/
    WdgEnNotification[1]();
  }
